...
<body class="bg-gray-900 text-gray-200 font-sans p-6">
    <div class="container mx-auto">
        ...
        $data = mysqli_query($koneksi, "SELECT * FROM apoteker WHERE code_apoteker='$code_apoteker'");
        while ($d = mysqli_fetch_array($data)) {
        ?>
            <div class="bg-gray-800 p-8 rounded-lg shadow-lg max-w-lg mx-auto">
                <h3 class="text-3xl font-semibold mb-4">Update Data Apoteker</h3>
                <form method="post" action="updateApoteker.php">
                    <!-- Hidden input for code_apoteker -->
                    <input type="hidden" name="code_apoteker" value="<?php echo htmlspecialchars($d['code_apoteker']); ?>">

                    <!-- Nama -->
                    <div class="mb-4">
                        <label for="namaApoteker" class="block text-lg font-medium mb-2">Nama</label>
                        <input type="text" id="namaApoteker" name="namaApoteker" value="<?php echo htmlspecialchars($d['namaApoteker']); ?>" required class="w-full p-2 rounded bg-gray-700 text-gray-200 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
