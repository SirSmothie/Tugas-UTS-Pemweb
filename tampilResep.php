...
<body class="bg-gray-900 text-gray-200 font-sans p-6">
    ...
    <div class="container mx-auto">
        ...
        <!-- Main Content -->
        <h2 class="text-3xl font-semibold text-center mt-8">Data Resep</h2>

        <!-- Add Button -->
        <div class="text-center my-4">
            <form method="POST" action="tambahResep.php" class="inline">
            ...
            <table class="min-w-full bg-gray-800 text-gray-200 border-collapse">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Tanggal Pembuatan</th>
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Deskripsi</th>
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Dokter</th>
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Obat</th>
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Apoteker</th>
                ...
                <tbody>
                    ...
                    $data = mysqli_query($koneksi, "
                        SELECT resep.*, 
                               dokter.namaDokter, 
                               obat.namaObat, 
                               apoteker.namaApoteker, 
                               pasien.namaPasien 
                        FROM resep
                        JOIN dokter ON resep.code_dokter = dokter.code_dokter
                        JOIN obat ON resep.code_obat = obat.code_obat
                        JOIN apoteker ON resep.code_apoteker = apoteker.code_apoteker
                        JOIN pasien ON resep.code_pasien = pasien.code_pasien
                    ");
                    ...
                    ?>
                        <tr class="hover:bg-gray-700 hover:text-white">
                            ...
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['tglPembuatan']); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['deskripsi']); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['namaDokter']??''); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['namaObat']??''); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['namaApoteker']??''); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['namaPasien']??''); ?></td>
                            <td class="border px-4 py-2">
                                <a href="ubahResep.php?code_resep=<?php echo $d['code_resep']; ?>" class="bg-yellow-600 hover:bg-yellow-700 text-white py-1 px-3 rounded shadow">Ubah</a>
                                <a href="hapusResep.php?code_resep=<?php echo $d['code_resep']; ?>" class="bg-red-600 hover:bg-red-700 text-white py-1 px-3 rounded shadow">Hapus</a>
                            </td>
                        </tr>
                    <?php 
                    }
