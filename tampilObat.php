...
<body class="bg-gray-900 text-gray-200 font-sans p-6">
    ...
    <div class="container mx-auto">
        ...
        <!-- Main Content -->
        <h2 class="text-3xl font-semibold text-center mt-8">Data Obat</h2>

        <!-- Add Button -->
        <div class="text-center my-4">
            <form method="POST" action="tambahObat.php" class="inline">
            ...
            <table class="min-w-full bg-gray-800 text-gray-200 border-collapse">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Code Obat</th>
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Nama Obat</th>
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Dosis</th>
                        <th class="px-4 py-2 bg-indigo-500 text-gray-100 border border-gray-700">Pabrik</th>
                ...
                <tbody>
                    ...
                    include 'koneksi.php';
                    $no = 1;
                    $data = mysqli_query($koneksi, "SELECT obat.*, pabrikobat.NamaPabrik FROM obat JOIN pabrikobat ON obat.code_pabrik = pabrikobat.code_pabrik");
                    while ($d = mysqli_fetch_array($data)) {
                    ?>
                        <tr class="hover:bg-gray-700 hover:text-white">
                            <td class="border px-4 py-2"><?php echo $no++; ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['code_obat']); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['namaObat']); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['dosis']); ?></td>
                            <td class="border px-4 py-2"><?php echo htmlspecialchars($d['NamaPabrik']??''); ?></td>
                            <td class="border px-4 py-2">
                                <a href="ubahObat.php?code_obat=<?php echo $d['code_obat']; ?>" class="bg-yellow-600 hover:bg-yellow-700 text-white py-1 px-3 rounded shadow">Ubah</a>
                                <a href="hapusObat.php?code_obat=<?php echo $d['code_obat']; ?>" class="bg-red-600 hover:bg-red-700 text-white py-1 px-3 rounded shadow">Hapus</a>
                            </td>
                        </tr>
                    <?php 
                    }
